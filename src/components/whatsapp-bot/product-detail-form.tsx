...
export function createEmptyProductRow(): ProductDetailRow {
  return {
...
export function ProductDetailForm({
  ...
  const PROMO_SOLD_OUT_MESSAGE =
    'A product can’t be on promo and sold out at the same time. Turn off one of these first.'

  const handlePromoChange = (checked: boolean) => {
    if (checked && row.soldOut) {
      toast.error(PROMO_SOLD_OUT_MESSAGE)
      return
    }
    onUpdate(row.id, { promo: checked })
  }

  const handleSoldOutChange = (checked: boolean) => {
    if (checked && row.promo) {
      toast.error(PROMO_SOLD_OUT_MESSAGE)
      return
    }
    onUpdate(row.id, { soldOut: checked })
  }
  ...
  return (
    <div className="panel flex flex-col flex-1 min-h-0 overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-5 space-y-5 min-h-0">
        ...
        <div>
          <p className="text-[11px] font-semibold text-ink-400 uppercase tracking-wider mb-2">Visibility</p>
            {row.detailsLoading ? (
            ...
            <div className="flex flex-col sm:flex-row sm:flex-wrap gap-3">
              ...
              <label className="flex items-center gap-2.5 cursor-pointer select-none rounded-xl border border-ink-100 bg-ink-50/50 px-4 py-3 flex-1 min-w-[10rem]">
                <input
                  type="checkbox"
                  checked={row.promo}
                  ...
                  className="w-4 h-4 rounded border-ink-300 text-brand-600 focus:ring-brand-500"
                />
                <span className="text-sm text-ink-700">Promo</span>
              </label>
              <label className="flex items-center gap-2.5 cursor-pointer select-none rounded-xl border border-ink-100 bg-ink-50/50 px-4 py-3 flex-1 min-w-[10rem]">
                <input
                  type="checkbox"
                  checked={row.preOrder}
                  onChange={e => onUpdate(row.id, { preOrder: e.target.checked })}
                  className="w-4 h-4 rounded border-ink-300 text-brand-600 focus:ring-brand-500"
                />
                <span className="text-sm text-ink-700">Pre-order</span>
              </label>
              <label className="flex items-center gap-2.5 cursor-pointer select-none rounded-xl border border-ink-100 bg-ink-50/50 px-4 py-3 flex-1 min-w-[10rem]">
                <input
                  type="checkbox"
                  ...
                  className="w-4 h-4 rounded border-ink-300 text-brand-600 focus:ring-brand-500"
                />
                <span className="text-sm text-ink-700">Sold out</span>
              </label>
            </div>
          )}
          <p className="text-[10px] text-ink-400 mt-2">Promo and Sold out can’t be on together.</p>
        </div>
        ...
        <ProductItemMediaSection company={company} itemId={row.isNew ? null : row.id} />
      ...
      <div className="px-4 sm:px-6 py-4 border-t border-ink-100 shrink-0 space-y-2 pb-[max(1rem,env(safe-area-inset-bottom))]">
        ...
        <div className="flex flex-col-reverse sm:flex-row gap-2">
          <button type="button" onClick={onCancel} disabled={saving} className="flex-1 btn-secondary">
            Cancel
          </button>
          <button
            type="button"
